...
MODELS = {
    "4.5": {
        ...
        "prompt": (
            ...
            "【指甲的精确定义】：指甲是位于每根手指最末端关节背面（即手指"
            "朝上一侧）的扁平硬质甲板，从甲根到指尖自由缘。只有这一小块"
            "区域才是指甲，其余任何皮肤表面都不是指甲。"
            "【必须逐像素保留】：手部轮廓、五根手指的完整形状与数量、"
            "每根手指的弯曲角度与姿势、皮肤纹理与肤色、戒指、"
            "背景、光照方向、阴影、构图、图像宽高比。"
            "【唯一允许修改】：每根手指甲面的颜色与图案，"
            "参考第二张图的美甲设计逐指替换。"
            "【严禁修改的区域】：指腹（手指内侧触摸面）、指侧皮肤、"
            "手背皮肤、指间缝隙、指关节与指节皮肤、手掌。"
            "这些区域即使看起来颜色均匀，也绝对不是指甲，禁止上色。"
            "【严禁】：重新生成手部结构、增减手指数量、改变手指姿势、"
            "改变手掌朝向、添加或移除戒指、改变拍摄角度。"
            "【特殊情况】：如果手掌朝向镜头（能看到掌心或指腹），"
            "则指甲不可见或仅露出很小边缘，此时不要在指腹上画指甲，"
            "保持原图不变。如不确定某区域是否为指甲，保持原图不变。"
        ),
    },
    "5.0": {
        ...
        "prompt": (
            ...
            "【指甲的精确位置】：指甲仅存在于每根手指最末端指节的背面"
            "（手指朝上一侧），是从甲根延伸到指尖的一小片扁平硬质甲板。"
            "指腹（触摸面）、指侧、手背皮肤、指间、关节处都不是指甲。"
            "唯一编辑区域：image_1 中每根手指甲面（指甲盖部分）。"
            "编辑内容：将每片指甲的颜色与图案替换为 image_2 中对应的美甲设计。"
            "严格保持不变：手部轮廓、五指完整、手指姿势、皮肤纹理与肤色、"
            "戒指、袖口、背景、光照、阴影、构图、宽高比。"
            "禁止在以下区域上色：指腹、指侧皮肤、手背非指甲皮肤、"
            "指间缝隙、指关节褶皱处、手掌。"
            "如手掌面朝镜头导致指甲不可见，不要在指腹上凭空绘制指甲，"
            "保持原图不变。禁止重新绘制手部任何非指甲区域。"
            "如不确定某区域是否为指甲，按原图保留。"
        ),
    },
}
